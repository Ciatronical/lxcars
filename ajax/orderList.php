...
function getOrderList( $data ){
    ...
    $sql = "SELECT distinct on ( ordnumber ) * FROM ( SELECT distinct on ( oe.ordnumber ) 'true'::BOOL AS instruction, oe.id, to_char( oe.itime, ".$format." ) AS itime, to_char(oe.mtime, ".$format." ) AS mtime, oe.ordnumber, instructions.description, COALESCE( NULLIF( oe.amount, null ) , 0 ) AS amount ";
