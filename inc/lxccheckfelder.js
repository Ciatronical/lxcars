...
function kz_to_lks(kz){
	...
	//alert(kza.indexOf(kz[0]));
	
	if( kza.indexOf(kz[0]) == -1 ){myalert('<em>LxCars Fehler</em>', "Zum Kennzeichen " + orgkz + " existiert kein Landkreis!", {left:350, top:20, width:300, height:100});};
	if( kza.indexOf(kz[0]) != -1 ){myalert('LxCars Information','Dieses Kennzeichen gehört zum Landkreis ' + lks[index] + '.', {left:300, top:20, width:300, height:100});};//lks[index]
...
function checkfelder(){
			if(valid_kz(document.car.c_ln.value) == -1 && document.car.chk_c_ln.checked){ alert("Kennzeichen vom Kraftfahrtbundesamt nicht vergeben!");}//
			if (!document.car.c_ln.value.match(/^[A-Z ÜÄÖ]{1,3}-[A-Z]{1,2}[0-9]{1,4}[H]{0,1}$/)&& document.car.chk_c_ln.checked) { myalert ('<em>LxCars Fehler!</em>', 'Kennzeichen fehlerhaft! Folgendes Format verwenden: MOL-RK73 oder MOL-DS88H für Oldtimer.', {button_title:'Habe ich verstanden!'}); return false; };
			if ((!(document.car.c_2.value.length <= 0)&&(!document.car.c_2.value.match(/^[0-9]{4}$/)))&& document.car.chk_c_2.checked){ myalert('<em>LxCars Fehler!</em>', 'Schlüsselnummer zu 2.2 fehlerhaft! Folgendes Format verwenden: 0600', {button_title:'Habe ich verstanden!'}); return false; };
			if ((!(document.car.c_3.value.length <= 0)&&(!document.car.c_3.value.match(/^([0-9A-Z]{3,10})$/))) && document.car.chk_c_ln.checked ) { myalert('<em>LxCars Fehler!</em>', 'Schlüsselnummer zu 2.3 fehlerhaft! Folgendes Format verwenden: ABL1277L3 oder 300', {button_title:'Habe ich verstanden!'}); return false; };
			if ((!(document.car.c_em.value.length <= 0)&&(!document.car.c_em.value.match(/^[0-9A-Z]{4,6}$/)))&& document.car.chk_c_em.checked) { myalert('<em>LxCars Fehler!</em>', 'Abgasschlüssel fehlerhaft! Folgendes Format verwenden: 0456 oder 010456', {button_title:'Habe ich verstanden!'}); return false; };
			if ((!(document.car.fin.value.length <= 0)&&(!checkfin(document.car.fin.value,document.car.cn.value))) && document.car.chk_fin.checked) { myalert('<em>LxCars Fehler!</em>', 'Fahrzeugidentnummer fehlerhaft! Folgendes Format verwenden: WDB2081091X123456. Prüfziffer nicht vergessen  Falls unbekannt \'-\' eingeben', {button_title:'Habe ich verstanden!'}); return false; };
			if (!(document.car.c_hu.value.length <= 0)&&(!document.car.c_hu.value.match(/^[\d]{1,2}[.][\d]{1,2}[.][\d]{0,4}$/))){myalert('<em>LxCars Fehler!</em>', 'Datum HU fehlerhaft! Folgendes Format verwenden: 1.2. oder 1.2.10', {button_title:'Habe ich verstanden!'}); return false; };
			if (!(document.car.c_d.value.length <= 0)&&(!document.car.c_d.value.match(/^[\d]{1,2}[.][\d]{1,2}[.][\d]{1,4}$/))){myalert('<em>LxCars Fehler!</em>', 'Zulassungsdatum fehlerhaft! Folgendes Format verwenden: 14.8.4', {button_title:'Habe ich verstandn!'}); return false; };
			var respKz = false;
